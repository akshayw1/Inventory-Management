...
class Admin:
    ...
    def admin_mainmenu(self,a,b):
         ...
         self.accounts.place(x=555, y=27)
         ...
         self.logout.place(x=1005, y=27)

         mi = PhotoImage(file="images/Door_Out-512.png")
         mi = mi.subsample(a, b)
         self.logout = Button(self.mainframe, text="Notifiy Restock", bd=5, font="roboto 11 bold", image=mi, compound=TOP)
         self.logout.image = mi
         self.logout.place(x=1075, y=27)
         ...
         self.changeuser.place(x=850, y=27)
         ...
         # Create a button for the "Store" option
         mi = PhotoImage(file="images/inventory.png")  # Replace "store.png" with your store icon image file
         mi = mi.subsample(a, b)
         self.store = Button(self.mainframe, text="Store", bd=5, image=mi, font="roboto 11 bold", compound=TOP,
                             command=self.open_store)
         self.store.image = mi
         self.store.place(x=700, y=27)  # Adjust the coordinates as needed

         # ... (continue with the rest of your buttons)
    ...
    def show_notification(self):
        # Replace this with the actual notification message you want to display
        notification_message = "You have a new notification!"

        # Show a pop-up notification with the received message
    ...
    def buildprodtable(self):
         ...
         self.cond=1
         self.mainsearch(1)
         Button(self.formframe, text="Reschedule Stock", font="robot 11 bold", bg="#FFFFFF", bd=5, width=15, height=2,
                command=self.show_stock_reschedule_popup).place(x=200, y=420)

    def show_stock_reschedule_popup(self):
        # Create a pop-up message
        result = messagebox.askquestion("Restore Stock", "Are you sure you want to reschedule stock?")

        if result == "yes":

                   messagebox.showinfo("Confirmation", "Stock Notification Send to store.")
        else:
    ...
    def open_store(self):
        # Hide other frames if necessary
        self.formframe1.place_forget()
        self.searchframe.place_forget()
        ...
        # Create StringVar variables for store details
        self.newstorename = StringVar()
        self.newstoreaddress = StringVar()
        self.newstorecity = StringVar()
        self.newstorestate = StringVar()
        self.newstorezip = StringVar()

        # Labels and Entry widgets for store details
        labels = ['Store Name', 'Address', 'City', 'State', 'ZIP']
        entry_vars = [self.newstorename, self.newstoreaddress, self.newstorecity, self.newstorestate, self.newstorezip]
        for i, label in enumerate(labels):
            Label(self.itemframe, text=label, font="Roboto 14 bold", bg="#ffffff").grid(row=i, column=0, pady=15,
                                                                                        sticky="w")
            Entry(self.itemframe, width=40, textvariable=entry_vars[i], font="roboto 11",
                  bg="#ffffff").grid(row=i, column=1, pady=15, padx=10, ipady=3)

        # Add store button and Back button
        Button(self.itemframe, text="Add Store", height=3, bd=6, command=self.insert_store, bg="#FFFFFF").grid(
            row=len(labels), column=1, pady=10, padx=12, sticky="w", ipadx=10)
        Button(self.itemframe, text="Back", height=3, width=8, bd=6, command=self.buildprodtable, bg="#FFFFFF").grid(
            row=len(labels), column=1, padx=16, pady=10, sticky="e", ipadx=10)

    def insert_store(self):
        # Retrieve store details from StringVar variables
        store_name = self.newstorename.get()
        store_address = self.newstoreaddress.get()
        store_city = self.newstorecity.get()
        store_state = self.newstorestate.get()
        store_zip = self.newstorezip.get()
        self.cur.execute('select * from products where product_id = ?',(int(self.newitemcode.get()),))

        # Insert store details into the 'stores' table using SQL queries
        insert_query = "INSERT INTO stores (store_name, store_address, store_city, store_state, store_zip) VALUES (?, ?, ?, ?, ?)"
        self.cur.execute(insert_query, (store_name, store_address, store_city, store_state, store_zip))

        # Optionally, you can clear the entry fields after insertion
        self.newstorename.set('')
        self.newstoreaddress.set('')
        self.newstorecity.set('')
        self.newstorestate.set('')
        self.newstorezip.set()
        self.cur.execute('select * from stores where store_address = ?',(int(self.newitemcode.get()),))

        # Provide feedback to the user (e.g., with a messagebox)
        # ...
        messagebox.showinfo("Confirmation", "Store added successfully to the database.")

        # You can also refresh the store table if needed
        # self.build_store_table()
